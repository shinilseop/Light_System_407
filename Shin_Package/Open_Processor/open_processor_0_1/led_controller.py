...
def  illum_match_needs(II_illum, avg_illum):
    ...
    if diff_value >= 0:
        ...
        if _direction==-1:
            control_led = led_down(diff_max_sensor, datas.sensor_influence_all[diff_max_sensor], datas.sensor_influence_value_all[diff_max_sensor], control_step)
        else:
    ...
    # 평균조도와 지점 조도의 차이가 음수인 경우 조도가 부족하므로 상향점등 후 해당 조명 하향점등 잠금
        ...
        if _direction==-1:
            control_led = led_up(diff_max_sensor, datas.sensor_influence_all[diff_max_sensor], datas.sensor_influence_value_all[diff_max_sensor], control_step)
        else:
            inf_led = datas.dir_inf[diff_max_sensor]['ind'][_direction]
            inf_value = datas.dir_inf[diff_max_sensor]['ind'][_direction + '_inf']
