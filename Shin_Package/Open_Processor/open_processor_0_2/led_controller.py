...
import Shin_Package.open_processor_0_2.led_controll as led
import Shin_Package.open_processor_0_2.led_selecter as selecter

# 잠금 초기화 하는 함수[조명 업,다운, 센서]
from Shin_Package.open_processor_0_2 import printer
...
def illum_match_needs(II_illum, avg_illum):
    ...
    if diff_value >= 0:
        ...
        print("LOCKING STATE : %s" % datas.led_up_lock[control_led])
    ...
    # 평균조도와 지점 조도의 차이가 음수인 경우 조도가 부족하므로 상향점등 후 해당 조명 하향점등 잠금
        ...
        print("LOCKING STATE : %s" % datas.led_down_lock[control_led])
