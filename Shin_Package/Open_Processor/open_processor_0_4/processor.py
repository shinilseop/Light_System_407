...
import Shin_Package.Open_Processor.open_processor_0_4.init_setter as initial
import Shin_Package.Open_Processor.open_processor_0_4.datas as datas
import Shin_Package.Open_Processor.open_processor_0_4.led_controller as controller
import Shin_Package.Open_Processor.open_processor_0_4.printer as printer
import Shin_Package.Open_Processor.open_processor_0_4.sensing as sensing
import Shin_Package.Open_Processor.open_processor_0_4.illum_adder as illum_adder
from Shin_Package.Open_Processor.open_processor_0_4 import led_controller
...
def process(start, case_num, change_time, illum_change_value):
    ...
    while datas.isFinish:
        ...
        ###### 계산조도의 적합 정도
        for i in range(9):
            if (datas.cal_lux[i] < 480) | (520 < datas.cal_lux[i]) | (490 > cal_avg) | (cal_avg > 510):
                datas.can_control = False
                break
        ...
        if datas.can_control:  ###### 조도가 만족했다면 계산조도로 전부 제어한 후 조도 재실측 후 실측값이 만족한지 테스트
            ...
            save_flag = True
            for i in range(9):
                if (datas.II_illum[i] < 480) | (520 < datas.II_illum[i]) | (avg_illum < 490) | (avg_illum > 510):
