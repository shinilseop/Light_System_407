...
import Shin_Package.Open_Processor_NL.Cal_Open_Processor_master.led_controll as led
import Shin_Package.Open_Processor_NL.Cal_Open_Processor_master.led_selecter as selecter
...
def illum_before_needs(II_illum, avg_illum):
    ...
    led_up(min_sensor, datas.sensor_influence_sum_part[min_sensor], datas.sensor_influence_value_sum_part[min_sensor],
           control_step)
...
def illum_after_needs(II_illum, avg_illum):
    ...
    if max_sensor == -1:
        ...
        led_down(max_sensor, datas.sensor_influence_sum_part_reverse[max_sensor],
                 datas.sensor_influence_value_sum_part_reverse[max_sensor], control_step)
...
def led_up(min_sensor, influence_rank, influence_value, control_step=1):
    ...
    #     # 선정 LED 한단계 상승
    if (datas.led_state[control_led_idx] + control_step) >= len(datas.led_control):
        print("Control step 조정 : %s -> %s"%(control_step,len(datas.led_control) - datas.led_state[control_led_idx] - 1))
        control_step = len(datas.led_control) - datas.led_state[control_led_idx] - 1
            ...
            if datas.led_down_lock[i] == 0:
                for j in range(len(datas.sensor_down_lock)):
                    if (datas.sensor_down_lock[j] != 0):
                        for k in range(len(datas.inf_ind[j])):
                            if datas.inf_ind[j][k] == i:
