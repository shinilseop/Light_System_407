import time
import serial
from Shin_Package.Open_Processor_NL.Open_Processor_NL_01 import datas
...
def blind_open():
    if datas.blind_deg < 0:
        ...
        blind_state = cmd_blind('1', 'R').decode()
        print(blind_state)
        blind_state = cmd_blind('2', 'R').decode()
        print(blind_state)
...
def blind_close():
    ...
    if datas.blind_deg == -70:
        print("BLIND DEGREE MAX, AND MIN LED. CHECK POINT.")
    else:
        control_deg = datas.blind_deg - (2.8 * datas.blind_accel)
        if control_deg < -70:
            control_deg = -70

        print("BLIND CLOSE(-2.8 degree)")
        # control_blind('1', 'A', '-70')
        # time.sleep(0.3)
        # control_blind('2', 'A', '-70')
        # time.sleep(0.3)
