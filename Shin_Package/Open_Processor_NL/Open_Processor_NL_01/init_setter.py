    ...
    for i in range(len(datas.dir_inf_temp)):
        ...
        for key, value in datas.dir_inf_temp[i]['ind'].items():
            ...
            for value_2 in datas.dir_inf_temp[i]['ind'][key]:
                print(value_2, i)
                print(len(datas.inf_df))
                print(datas.inf_df['ind'])
                print(datas.inf_df['ind'][i])
                print(datas.inf_df['ind'][i]['led'] == value_2)
                print(datas.inf_df['ind'][i].loc[datas.inf_df['ind'][i]['led'] == value_2])
                print((datas.inf_df['ind'][i].loc[datas.inf_df['ind'][i]['led'] == value_2]['led'].tolist()[0]))
                result_df = result_df.append(
                    {'led': datas.inf_df['ind'][i].loc[datas.inf_df['ind'][i]['led'] == value_2]['led'].tolist()[0],
                     'value': datas.inf_df['ind'][i].loc[datas.inf_df['ind'][i]['led'] == value_2]['value'].tolist()[
                         0]},
                    ignore_index=True)
