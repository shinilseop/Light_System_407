...
import Shin_Package.Open_Processor_NL.Open_Processor_NL_02.led_controll as led
import Shin_Package.Open_Processor_NL.Open_Processor_NL_02.led_selecter as selecter
...
def illum_match_needs(II_illum, avg_illum):
    ...
    # 평균조도와 지점 조도의 차이가 음수인 경우 조도가 부족하므로 상향점등 후 해당 조명 하향점등 잠금
        ...
        ### 블라인드 제어 불가능 상태일 때
        if ((not datas.more_illum_use_blind) | (datas.blind_lock > 0) | (datas.blind_deg == -70)):
            # 조명 선정 및 제어
