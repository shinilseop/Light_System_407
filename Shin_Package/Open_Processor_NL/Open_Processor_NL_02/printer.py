...
def save_data(start, avg_cct, avg_illum, sum_curr, uniformity, get_times,
    ...
    df_record[len(df_record) - 1].loc[0, 'blind_deg'] = datas.blind_deg
    df_record[len(df_record) - 1].loc[0, 'blind_lock'] = datas.blind_lock
    df_record[len(df_record) - 1].loc[0, 'blind_position'] = datas.blind_position
    ...
    df_record[len(df_record) - 1].to_csv(
        "%s\\%s\\[%s]_step_%s_%s_E%s_U%s.csv" % (
            datas.path, datas.save_folder, get_times, save_name, control_object, int(sum_curr), round(uniformity, 3)))
    print(start, "->", step_end)

def print_all(avg_cct, avg_illum, sum_curr, uniformity):
    # 현재 데이터 상태 출력
    # print_control_lux()
    ...
    print_need_value(avg_cct, avg_illum, sum_curr, uniformity)

def print_blind_state():
    print("Blind State : Degree(%s), Position(%s), Locked(%s)" % (
    datas.blind_deg, datas.blind_position, datas.blind_lock))

def print_need_value(avg_cct, avg_illum, sum_curr, uniformity):
    print("Avg CCT : %s,   Avg Illum : %s,   Sum Curr : %s,   Uniformity : %s"%(avg_cct, avg_illum, sum_curr, uniformity))
    print("Blind State : Degree(%s), Position(%s), Locked(%s)"%(datas.blind_deg, datas.blind_position, datas.blind_lock))

def print_led_state():
    print("%42s%31s%31s"%('Control Lux', 'LED UP LOCK', 'LED DOWN LOCK'))
    for idx in range(1, len(datas.led_state), 6):
        for j in range(0, 6):
            print("%07s" % round(datas.led_control_lux[datas.led_state[idx+j]], 2), end='')
        print('|', end='')
        for j in range(0, 6):
