...
import Shin_Package.led_select_module_blind_not_include.led_controll as led
import Shin_Package.led_select_module_blind_not_include.led_selecter as selecter
...
# 시뮬레이션 용도
def control_led(led_num, control_data, mode):
    led_lock_minus()

    datas.led_state_index[led_num][1] = control_data[0]
    datas.led_state_index[led_num][2] = control_data[1]
    datas.led_state_index[led_num][3] = control_data[2]
    datas.led_state_index[led_num][4] = control_data[3]
    change_lux = control_data[4] - datas.led_state_lux[led_num]
    datas.led_state_lux[led_num] = control_data[4]
...
def illum_before_needs(II_illum, avg_illum):
    ...
    if min_sensor == -1:
        print('cant control this process')
    else:
        print('SELECT %s Position' % (min_sensor + 1))
        control_step = abs(datas.object_illum - II_illum[min_sensor]) * 0.7
...
def illum_match_needs(II_illum, avg_illum):
    ...
    '''* control_step *'''
    control_step = abs(datas.object_illum - II_illum[diff_max_sensor]) * 0.7
...
def illum_after_needs(II_illum, avg_illum):
    ...
    if max_sensor == -1:
        ...
        control_step = abs(datas.object_illum - II_illum[max_sensor]) * 0.7
...
def led_down(max_sensor, influence_rank, influence_value, lux_diff_70per):
    # 잠금 및 제어가능 점검
    ...
    '''* 제어지표 찾기 *'''
    ch1, ch2, ch3, ch4, lux = 0, 0, 0, 0, 0
    datas.lux_stand = lux_diff_70per
    datas.cct_stand
    datas.control_df
    ...
    # 선정 LED n단계 다운
    print(
        f'down Sensor[{max_sensor + 1}] LED[{control_led_idx}] [{datas.led_state_index[control_led_idx]}]({datas.led_state_lux[control_led_idx]}) -> [{[ch1, ch2, ch3, ch4]}]({lux})')

    control_led(control_led_idx, [ch1,ch2,ch3,ch4, lux], 'down')
...
def led_up(min_sensor, influence_rank, influence_value, lux_diff_70per):
    # 제어할 LED 선정
    # 잠금 및 제어가능 점검
    ...
    '''* 제어지표 찾기 lux_diff_70per 기반으로 찾아야됨*'''
    ch1, ch2, ch3, ch4, lux = 0, 0, 0, 0, 0
    '''***************'''

    #     # 선정 LED 한단계 상승
    print(
        f'up Sensor[{min_sensor + 1}] LED[{control_led_idx}] [{datas.led_state_index[control_led_idx]}]({datas.led_state_lux[control_led_idx]}) -> [{[ch1, ch2, ch3, ch4]}]({lux})')

    control_led(control_led_idx, [ch1,ch2,ch3,ch4, lux], 'up')
