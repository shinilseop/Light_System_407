...
def select_control_led(sensor, influence_rank, isLow):
    ...
    '''*최소 혹은 최고 상태인지 아닌지는 판단 제거(추후 추가 예정)*'''
    for i in range(len(influence_rank)):
        if (isLow) & (datas.led_up_lock[influence_rank[i]] == 0) & (datas.led_state[influence_rank[i]]):
            control_led_idx = influence_rank[i]
            control_led_rank = i
            print("SELECT UP LED NUM : %s" % control_led_idx)
            break
        elif (not isLow) & (datas.led_down_lock[influence_rank[i]] == 0) & (datas.led_state[influence_rank[i]]):
            control_led_idx = influence_rank[i]
            control_led_rank = i
...
def find_similar_influence(sensor, now_rank, influence_rank, influence_value, isLow):
    ...
    for j in range(control_led_rank + 1, len(influence_rank)):
        if (influence_value[control_led_rank] - datas.similar_led_range <=
            ...
            if (isLow):
                if (datas.led_state_lux[influence_rank[control_led_rank]] > datas.led_state_lux[influence_rank[j]]) & (
                        datas.led_up_lock[influence_rank[j]] == 0) & (datas.led_state[influence_rank[j]]):
                    control_led_idx = influence_rank[j]
                    control_led_rank = j
                    print("CHANGE(SIMILAR) LED NUM : %s" % control_led_idx)
            else:
                if (datas.led_state_lux[influence_rank[control_led_rank]] < datas.led_state_lux[
                    influence_rank[j]])  & (datas.led_down_lock[influence_rank[j]] == 0) & (datas.led_state[influence_rank[j]]):
                    control_led_idx = influence_rank[j]
                    control_led_rank = j
                    print("CHANGE(SIMILAR) LED NUM : %s" % control_led_idx)
